...
void cargar_datos(int , int ,  int *, int *,  int );
void mostrar_resultados( int *, int *, int );
...
void cargar_datos(int cx, int cy,  int *vecx, int *vecy,  int j)
{
	*(vecx+j)=cx;
	*(vecy+j)=cy;
...
void mostrar_resultados( int *vecx, int *vecy, int i)
{
	int cg=0, ca=0,  j;
                for (j=0;j<i;j++)  // me conviene usar for para recorrer vectores
           
             	{
             		...
             		if ( *(vecx+j) >=0 && *(vecx+j)<=500 
					    && *(vecy+j)>=0 && *(vecy+j)<=250 ) {printf("GOL\n"); cg=cg+1;}
             		else {printf("AFUERA\n");ca=ca+1;}           						          		
